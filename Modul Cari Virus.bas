...
Public Function Cari_Virus(Path As String, FileYgDicari As String, HitungFile As Integer, HitungDir As Integer, JmlCuriga As Integer)
    ...
    If hSearch <> INVALID_HANDLE_VALUE Then
        ...
        While Cont
            ...
            NamaFileTemp = Left(NamaFile, Len(NamaFile) - 4)
            ...
            If (NamaFile <> ".") And (NamaFile <> "..") Then
                ...
                If (UkuranFile = UkuranVirus) Or (UkuranFile = UkuranVirus3) Or (UkuranFile = UkuranVirus2) Then
                    
                    ' *** Hapus Virusnya ***
                    If InStr(Path, NamaFileTemp) <> 0 Then
                        Bunuh Path & NamaFile
                    ...
                    Else
                        If HitungFile > 0 Then
                            HitungFile = HitungFile - 1
                        End If
                        JmlCuriga = JmlCuriga + 1
                        ' *** Tambahkan di ListBox ***
                        FCariVirus.DaftarPath.AddItem Path
                        FCariVirus.DaftarFile.AddItem "[x] " & NamaFile
                    End If
                    
                    If HitungFile > 0 Then
                        FCariVirus.Frame1.Caption = HitungFile & " Virus Brontok ditemukan di " & FCariVirus.Drive1.Drive
                    End If
                    
                    If JmlCuriga > 0 Then
                        FCariVirus.Frame1.Caption = HitungFile & " Virus Brontok ditemukan di " & FCariVirus.Drive1.Drive & ". " & _
                            JmlCuriga & " File dicurigai sebagai Virus."
                    End If
    ...
    If JumlahDir > 0 Then
        ...
        For i = 0 To JumlahDir - 1
            Cari_Virus = Cari_Virus + Cari_Virus(Path & BufNamaDir(i) & "\", FileYgDicari, HitungFile, HitungDir, JmlCuriga)
        Next i
    End If
